...
public interface VehicleRepository extends JpaRepository<Vehicle, Long> {
        ...
        @Query("SELECT v.model.id AS modelId, COUNT(v) AS vehicleCount FROM Vehicle v GROUP BY v.model.id")
        List<Object[]> countVehiclesByModel();

        @Query("SELECT m.brand.id AS brandId, COUNT(v) AS vehicleCount FROM Vehicle v " +
                        "LEFT JOIN v.model m " +
                        "GROUP BY m.brand.id")
        ...
        @Query("SELECT c.id AS ColorId, COUNT(v) AS vehicleCount FROM Vehicle v " +
        "LEFT JOIN v.color c " +
        "LEFT JOIN v.model m " +
        "LEFT JOIN v.configurations conf " +
        "WHERE (:#{#vehicleFilterDTO.colorIds} IS NULL OR c.id IN (:#{#vehicleFilterDTO.colorIds})) " +
        ...
        @Query("SELECT conf.id AS ConfigurationId, COUNT(v) AS vehicleCount FROM Vehicle v " +
        "LEFT JOIN v.color c " +
        "LEFT JOIN v.model m " +
        "LEFT JOIN v.configurations conf " +
        "WHERE (:#{#vehicleFilterDTO.colorIds} IS NULL OR c.id IN (:#{#vehicleFilterDTO.colorIds})) " +
        ...
        @Query("SELECT m.id AS ModelId, COUNT(v) AS vehicleCount FROM Vehicle v " +
        "LEFT JOIN v.color c " +
        "LEFT JOIN v.model m " +
        "LEFT JOIN v.configurations conf " +
        "WHERE (:#{#vehicleFilterDTO.colorIds} IS NULL OR c.id IN (:#{#vehicleFilterDTO.colorIds})) " +
